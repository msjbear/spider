...
public class SpiderThread extends Thread {
    ...
    public void codeProcess() {
        ...
        for (int i = 0; i < 3; i++) { // 最多发送3次请求
            try {
                ...
                logger.error("send request url : " + urlReq.getUrl() + " error! ", e);
            }
            try {
                Thread.sleep(1000);
        ...
        if (!StringUtil.isEmpty(html)) { // html 若无内容 则无需分析
            ...
            for (AnalyserTemplate template : templates) {
                ...
                List<Conversation> convsTemp = template.findConversations(urlReq, html, docHtml);
                if (convsTemp != null)
                    for (Conversation convs : convsTemp) {
                        ...
                        if (convs.getRunable().equals("1"))
                            urlReqResult.add(new UrlRequest(formatUrl(convs.getSelfLink(), urlReq), urlReq.getTask()));
                    }
            ...
            if (urlReqResult != null && urlReqResult.size() > 0) {
                ...
                logger.info("url : " + urlReq.getUrl() + " get next url : " + urlReqResult.size());
            }
            if (convsResult != null && convsResult.size() > 0) {
                ClientMessageCenter.getInstance().resultSend(convsResult);
                logger.info("url : " + urlReq.getUrl() + " get convsResult : " + convsResult.size());
            }
        }
        ...
        ClientMessageCenter.getInstance().stateSend(
                new UrlState(html.length(), urlReqResult.size(), convsResult.size(), urlReq));
    }
    ...
    public static String sendRequest(UrlRequest urlReq) throws Exception {
        ...
        if (urlReq.getMethod().toLowerCase().endsWith("get")) {
            ...
            try {
                ...
                if (urlReq.getHeader() == null || !urlReq.getHeader().containsKey("User-Agent")) {
                    httpGet.addHeader("User-Agent", userAgents.get(new Random().nextInt(userAgents.size())));
                }
     ...
     * @throws IOException
     */
    private List<AnalyserTemplate> findAnalyserTemplate(UrlRequest urlReq) throws XPathExpressionException,
            IOException, SAXException {
        List<AnalyserTemplate> templates = new ArrayList<AnalyserTemplate>();

        Document doc = XmlUtil.formatToDoc(FileUtil.readStream(
                this.getClass().getResourceAsStream("/plugins/" + getUrlDomain(urlReq) + ".xml"), "utf-8"));

        NodeList nodes = (NodeList) xpath.evaluate("//TEMPLATE", doc, XPathConstants.NODESET);
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            try {
                if (!StringUtil.isEmpty(classname)) {
                    templates.add((AnalyserTemplate) Class.forName(classname).newInstance());
                } else {
                    templates.add(new XmlAnalyserTemplate(nodes.item(i), urlReq));
                }
            } catch (Exception e) {
                logger.error("analyser template error!  \n node xml is : {}" + XmlUtil.printNodeXml(nodes.item(i)), e);
            }
        }
